// issue: unexpected playback reset
...
function createNewInputList() {
  ...
  if (newFiles.length > 0) {
    ...
    const fileEntries = newFiles.map(
      (file) => `file '${path.join(VIDEO_DIR, file)}'`
    )
...
function startFFmpeg(inputListFile) {
  ...
  ffmpegProcess.on('close', (code) => {
    ...
    if (code === 0) {
      console.log(
        'FFmpeg finished streaming all files. Waiting for new files...'
      )
      startFFmpeg(`${INPUT_LIST_BASE}_${ffmpegListCounter}.txt`)
      ffmpegListCounter++
    }
  })
}
